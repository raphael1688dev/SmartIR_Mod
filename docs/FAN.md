...
> **SmartIR(Mod) note** — For new setups, prefer the **Config Flow UI** (Settings → Devices & Services → + Add Integration → SmartIR(Mod) → Fan). YAML below still works and auto-imports to a Config Entry on first run; remove the YAML block once the entity is confirmed. Sensor / delay options can be edited later via the integration card's ⚙️ Options. For opt-in multi-HA state sync via MQTT, see [root README](../README.md#multi-ha-state-sync-optional).
